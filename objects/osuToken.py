...
class token:
	def __init__(self, userID, token_ = None, ip ="", irc = False, timeOffset = 0, tournament = False, friendPM = False):
		"""
		...
		self.messagesBuffer = []
		self.BlockNonFriendPM = friendPM
		# Default variables
		self.spectators = []
	...
	def joinChannel(self, channelObject):
		...
		if channelObject.publicRead == False and self.admin == False and self.privileges & privileges.USER_DONOR <= 0 and channelObject.name != "#admin":
