...
def getCountry(ip):
	...
	try:
		...
		result = json.loads(urllib.request.urlopen("{}/{}".format(glob.conf.config["localize"]["ipapiurl"], ip), timeout=15).read().decode())["country"]
		return result.upper()
	except:
...
def getLocation(ip):
	...
	try:
		...
		result = json.loads(urllib.request.urlopen("{}/{}".format(glob.conf.config["localize"]["ipapiurl"], ip), timeout=15).read().decode())["loc"].split(",")
