...
def loginBanned():
	...
	packets += notification("You are banned.")
	return packets

def loginLocked():
	packets = packetHelper.buildPacket(packetIDs.server_userID, [[-1, dataTypes.SINT32]])
	packets += notification("Your account is locked.")
	return packets
...
def InviteUser(sender, senderId, target, message):
	return packetHelper.buildPacket(packetIDs.server_invite, [[sender, dataTypes.STRING], [message, dataTypes.STRING], [target, dataTypes.STRING], [senderId, dataTypes.UINT32]])

def UserPMBlocked(sender, senderId, target, message):
	return packetHelper.buildPacket(packetIDs.Bancho_UserPMBlocked, [[sender, dataTypes.STRING], [message, dataTypes.STRING], [target, dataTypes.STRING], [senderId, dataTypes.UINT32]])
